...
@section('content')
    ...
    <p>See all registered payment methods.</p>
    <div class="StockLinks">
        <a href="{{ route('paymentmethods.create') }}"><span style="font-size: 30px">+</span>Add Payment Method</a>
    </div>
    <table>
        <thead>
            <tr>
            ...
            @foreach($paymentMethods as $method)
                <tr>
                    ...
                    <td style="display:flex; gap:8px;">
                        {{-- EDIT --}}
                        <a href="{{ route('paymentmethods.edit', $method->id) }}"
                        style="background:#f0ad4e; color:white; padding:5px 10px; border-radius:5px; text-decoration:none;">
                            Edit
                        </a>

                        {{-- DELETE --}}
                        <form action="{{ route('paymentmethods.destroy', $method->id) }}" method="POST"
    ...
    <div style="display:flex; gap:8px; align-items:center;">
        ...
        @else
            <a href="{{ $paymentMethods->previousPageUrl() }}" style="opacity:0.5; font-weight:bold; font-size:40px;">←</a>
        @endif
        ...
        @if ($paymentMethods->hasMorePages())
            <a href="{{ $paymentMethods->nextPageUrl() }}" style="opacity:0.5; font-weight:bold; font-size:40px;">→</a>
        @else
            <span style="opacity:0.5; font-weight:bold; font-size:40px;">→</span>
        @endif
