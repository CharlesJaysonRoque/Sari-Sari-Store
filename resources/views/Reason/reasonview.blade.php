...
@section('content')
    ...
    @endif
    <h1>Reason</h1>
    <p>See all registered stock-out reasons.</p>
    <div class="StockLinks">
        <a href="{{ route('reasons.create') }}"><span style="font-size: 30px">+</span>Add Reason</a>
    </div>
    <table>
        <thead>
            <tr>
    ...
    <div style="display:flex; gap:8px; align-items:center;">
        ...
        @else
            <a href="{{ $reasons->previousPageUrl() }}" style="opacity:0.5; font-weight:bold; font-size:40px;">←</a>
        @endif

        {{-- Page Numbers --}}
        @foreach ($reasons->getUrlRange(1, $reasons->lastPage()) as $page => $url)
            @if ($page == $reasons->currentPage())
        ...
        @if ($reasons->hasMorePages())
            <a href="{{ $reasons->nextPageUrl() }}" style="opacity:0.5; font-weight:bold; font-size:40px;">→</a>
        @else
            <span style="opacity:0.5; font-weight:bold; font-size:40px;">→</span>
        @endif
